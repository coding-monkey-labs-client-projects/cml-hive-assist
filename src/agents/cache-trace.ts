...
function resolveCacheTraceConfig(params: CacheTraceInit): CacheTraceConfig {
  ...
  const includePrompt =
    parseBooleanValue(env.CML_HIVE_ASSIST_CACHE_TRACE_PROMPT) ?? config?.includePrompt;
  const includeSystem =
    parseBooleanValue(env.CML_HIVE_ASSIST_CACHE_TRACE_SYSTEM) ?? config?.includeSystem;
